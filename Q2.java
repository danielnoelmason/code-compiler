...
public class Q2 {
	...
	public static void main(String[] args) {
		...
		/*
		 * the following is the example program being turned into a string and then
		 * scanned to produce a output of line numbers, token types and token names.
		 * this is done using the new readFile() and scan() method
		 */

		String Q2Example = readFile("src/Q2Example.java");
		scan(Q2Example);
	...
	public static void scan(String text) {
		...
		while (index < textLength) {
			...
			// Operations
				...
				// following is for identifying any of the string operators
				if (getOP(currentCh) != null || currentCh == '&' || currentCh == '|') {
					...
					index++;
				} // following is to identify char operators
				else {
			...
			// Integers and Doubles
				...
				if (number.length() >= 1) {
					// the following sees if a digit is a double or a integer
					// also checks if number string contains '_' or a letter, if so, stored as
					// identifier
					if (number.contains(".")) {
			...
			// Letters (Key Words, Identifiers and Strings)
				...
				// the following if statements check if the token name is an keyword, boolean
				// value else its an identifier
			...
			//the following if statements see if the token name is a string
			else if (currentCh == '"') {
				String str = "";
				str = str + currentCh;
				index++;
			...
			//the following else will occur for anything outside of weeJava e.g. '$'
			else {
	...
	//the copied methods and enum from Q1
