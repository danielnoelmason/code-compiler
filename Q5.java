...
public class Q5 {
	...
	public static void main(String[] args) {
		...
		String Q5Example = readFile("src/Q5Example.java");// variable name incorrect WORKS
		scan(Q5Example);
	...
	//Q5 The following is the method to get all the int variable names and the integers
	public static void q5(String[] tokenName, String[] tokenType, int[] lineNumber) {
		
		for (int i = 0; i < tokenType.length; i++) {
			if (tokenType[i] == TokenType.KEYWORD_INT.toString()) {
				i++;
				System.out.print(tokenName[i]);
				for (int j = i; lineNumber[j] ==lineNumber[i]; j++) {
					if(tokenType[j]==TokenType.INTEGER.toString()) {
						System.out.println(": "+tokenName[j]);
						System.out.println();
					}
				}
			} else {
...
//4D This checks if a bracket has been closed 
	...
	public static void scan(String text) {
		...
		q5(tokenName, tokenType, lineNumber);
